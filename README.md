...
- [x] **Layout totalmente responsivo (Mobile-First / Media Queries):** O layout adapta-se de forma fluida e elegante para dispositivos móveis, tablets, notebooks e telas desktop de alta resolução, utilizando breakpoints estruturados e flexbox/grid CSS.
...
- [x] **Boas práticas de SEO básico:** Implementação utilizando tags semânticas do HTML5 (`<header>`, `<main>`, `<section>`, `<article>`, `<footer>`), otimização de títulos e meta descrições para cada página, além de atributos de segurança e acessibilidade (`target="_blank"`, `rel="noopener noreferrer"`, `aria-label`).
...
josev001-portfolio/
├── css/
│   ├── animations.css     # Definição de keyframes e transições de entrada
│   ├── responsive.css     # Media queries para ajuste responsivo nos breakpoints
│   └── style.css          # Estilo global, tokens de cores e componentes estruturais
├── img/
